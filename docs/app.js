...
const GITHUB_RESOURCE_BASE = "https://github.com/psidavidflores/paquete-intervencion/tree/main/recursos";
const githubFolderById = {
  atencion: "atencion",
...
function resourcePage(resource) {
  const folder = githubFolderById[resource.id];
  return folder ? `${GITHUB_RESOURCE_BASE}/${folder}` : resource.page || "";
}
...
function cardTemplate(resource) {
  ...
  const resourceAction = page
    ? `<a class="card-link" href="${escapeHtml(page)}" target="_blank" rel="noopener">Abrir carpeta <span aria-hidden="true">→</span></a>`
    : `<button class="card-link" type="button" data-preview="${escapeHtml(resource.id)}">Ver detalle <span aria-hidden="true">→</span></button>`;
  return `
